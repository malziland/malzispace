...
| Lock-flow E2E | `npm run test:e2e:lock` (step 11/11) | 2026-07-16 ported to the mode switch; locally port-blocked (foreign emulator on :5001) — verified via CI run on push, see CI row |
| Protect E2E suites | `test:e2e:protect`, `test:e2e:protect:fuzz` | 2026-07-16 [NOT RUN locally — same port block]; changed modules covered by green simulator/mobile suites; CI-independent stack, run before next protect-related release |
...
| CI pipeline | `.github/workflows/verify.yml` (hardened, 3 jobs) | pending: first run on the retrofit push — update this row with the run ID |
...
key decision, CI run ID, protect suites vs. port block) — each with an owner.

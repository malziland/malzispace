...
async function testToolbar(modeName, contextOptions) {
  ...
  try {
    ...
    const fixedSeeds = modeName === 'desktop' ? [11, 29] : [7];
    for (const seed of fixedSeeds) {
      results.push(await runWildInteractionScenario(page, modeName, seed, consoleIssues, pageErrors));
    }
    const randomSeedBase = Date.now() ^ (Math.random() * 0xFFFFFF >>> 0);
    const randomCount = modeName === 'desktop' ? 2 : 1;
    for (let i = 0; i < randomCount; i++) {
      const rndSeed = (randomSeedBase + i * 9973) >>> 0;
      results.push(await runWildInteractionScenario(page, modeName, rndSeed, consoleIssues, pageErrors));
    }

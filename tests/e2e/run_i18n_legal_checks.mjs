...
async function checkLanding(page, results) {
  ...
  const hero = (await textOf(page, '.hero-copy p')) || '';
  const button = (await textOf(page, '#createForm button')) || '';
  const privacyCard = (await page.locator('.features > div').nth(2).textContent()) || '';
  ...
  results.push({ name: 'landing privacy card no false no-database claim', pass: !/keine datenbank/i.test(privacyCard) && /No signup/i.test(privacyCard), detail: privacyCard.trim() });
...
async function checkSpace(page, results) {
  ...
  const footerPrivacy = (await page.locator('.site-footer a').first().textContent()) || '';
  ...
  results.push({ name: 'space footer translated', pass: /Privacy/i.test(footerPrivacy), detail: footerPrivacy.trim() });
}
...
async function checkPrivacy(page, results) {
  ...
  const updated = (await textOf(page, '.legal-meta')) || '';
  const servicesText = (await page.locator('.legal-section').nth(3).textContent()) || '';
...
async function checkTerms(page, results) {
  ...
  const heading = (await textOf(page, 'h1')) || '';
  const meta = (await textOf(page, '.legal-meta')) || '';
  ...
  results.push({ name: 'terms meta translated', pass: /Service: malziSPACE/i.test(meta), detail: meta.trim() });
  results.push({ name: 'terms section translated', pass: /Scope and provider/i.test(section), detail: section.trim() });
}
...
async function checkImprint(page, results) {
  ...
  const heading = (await textOf(page, 'h1')) || '';
  const body = (await page.locator('.legal-card').textContent()) || '';
